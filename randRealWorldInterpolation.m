...
% model data set
    ...
    plot_methods(x, y, x, 'Randomly Sampled Google Pixel 5 Screen on Time v Battery');
    ...
    plot_methods(x1, y1, x1, 'Randomly Sampled iPhone 12 Screen on Time v Battery');
    disp('Iphone data:' );
    disp(w1)
    ...
    plot_methods(x2, y2, x2, 'Randomly Sampled One Plus 9 Screen on Time v Battery');
    ...
    plot_methods(x3, y3, x3, 'Randomly Sampled Samsung Galaxy 21 Screen on Time v Battery');
    ...
    plot_methods(x4, y4, x4, 'Randomly Sampled Xiaomi Screen on Time v Battery');
    disp('Xiaomi data:' );
    disp(w4)
end
...
% model data set
    ...
    y = w(:,5); % battery drain 
    eval_inputs = linspace(min(x), max(x));
    plot_methods(x, y, x, 'Randomly Sampled Google Pixel 5 App Usage Time v Battery');
    disp('google pixel data:')
    disp(w)
    ...
    y1 = w1(:,5);
    eval_inputs1 = linspace(min(x1), max(x1));
    plot_methods(x1, y1, x1, 'Randomly Sampled iPhone 12 App Usage Time v Battery');
    disp('Iphone data:' );
    disp(w1)
    ...
    y2 = w2(:,5);
    eval_inputs2 = linspace(min(x2), max(x2));
    plot_methods(x2, y2, x2, 'Randomly Sampled One Plus 9 App Usage Time v Battery');
    disp('One Plus 9 data:' );
    disp(w2)
    ...
    y3 = w3(:,5);
    eval_inputs3 = linspace(min(x3), max(x3));
    plot_methods(x3, y3, x3, 'Randomly Sampled Samsung Galaxy 21 App Usage Time v Battery');
    disp('Samsung Galaxy data:' );
    disp(w3)
    ...
    y4 = w4(:,5);
    eval_inputs4 = linspace(min(x4), max(x4));
    plot_methods(x4, y4, x4, 'Randomly Sampled Xiaomi App Usage Time v Battery');
    disp('Xiaomi data:' );
    disp(w4)
end
...
% To somewhat alleviate high-sensitivity in the Vandermonde, Newton, 
% and Lagrange interpolants, instead of taking the entire data set, 
% randomly take 15 rows from the data set and use those instead 
random_samples = 15;
